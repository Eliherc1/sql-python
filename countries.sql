...
/*1. ¿Qué consulta harías para obtener todos los países que hablan esloveno? Tu consulta debe devolver el nombre del país, el idioma y 
el porcentaje de idioma. Tu consulta debe organizar el resultado por porcentaje de idioma en orden descendente. (1) */

SELECT con.name, len.language , len.percentage
FROM countries con
JOIN languages len ON con.id=len.country_id
WHERE len.language = 'Slovene'
ORDER BY len.percentage DESC;

/*2. ¿Qué consulta ejecutarías para mostrar el número total de ciudades para cada país? Su consulta debe devolver el nombre del país y el número 
total de ciudades. Tu consulta debe organizar el resultado por el número de ciudades en orden descendente. (3) */

SELECT con.name, COUNT(*)
FROM countries con
JOIN cities cit ON con.id=cit.country_id
GROUP BY con.name
ORDER BY COUNT(*) DESC;

/*3. ¿Qué consulta harías para obtener todas las ciudades de México con una población de más de 500,000? Tu consulta debe organizar el 
resultado por población en orden descendente. (1) */

SELECT cit.name , cit.population
FROM countries con
JOIN cities cit ON con.id=cit.country_id
WHERE con.name= 'Mexico'
...
SELECT con.name nombre_pais ,cit.name nombre_ciudad, cit.district distrito, cit.population poblacion
FROM countries con
JOIN cities cit ON con.id=cit.country_id
WHERE con.name= 'Argentina'
